...
def button_handler(update: Update, context: CallbackContext):
    ...
    if query.data == 'buy':
        ...
        keyboard = [[
            InlineKeyboardButton("📤 Submit Screenshots",
...
def handle_photos(update: Update, context: CallbackContext):
    ...
    if user_state.get(user_id) == "collecting_screenshots":
        ...
        if user_screenshot_counter[user_id] == 3:
            ...
            keyboard = [[
                InlineKeyboardButton("📥 Send Payment Receipt",
...
# ✅ Handle Documents (image documents like jpg/png)
def handle_documents(update: Update, context: CallbackContext):
    ...
    if file_name.endswith(('.jpg', '.jpeg', '.png')):
        # Trick: simulate photo for handler reuse
        ...
        context.bot.send_message(
            chat_id=user_id,
            text=
            "❌ Unsupported file type. Please send only images (JPG/PNG) as screenshots or payment proof."
        )
...
def submit_command(update: Update, context: CallbackContext):
    update.message.reply_text(
        "📤 Please click 📤 Submit Screenshots option at top")
